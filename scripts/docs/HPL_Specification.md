...
- 类型系统严格区分 `bool` 和 `int`，在循环次数、表单引用索引等场景会拒绝布尔值
- 异步回调（`utils.async_run_func`/`utils.async_run_cmd`）中的异常会被静默丢弃，不产生错误信息
...
> 
> **词法与语法处理**：
> - 词法阶段：点号 `.` 不是 token 分隔符，因此 `math.sqrt` 被整体解析为单个 `TOKEN_ID_WORD`，payload 为 `"math.sqrt"`
> - 语法阶段：`{func, ...}` 的解析器直接读取 `TOKEN_ID_WORD` 的 payload 作为函数名，**不经过变量名的校验逻辑**
> - 这种设计允许函数名使用点号进行模块化命名，同时保持变量名的简洁性
> 
> **对比**：
> - `x = math.sqrt` — 错误！尝试将 `math.sqrt` 作为变量名（表达式上下文）
> - `{func, math.sqrt(4)}` — 正确！`math.sqrt` 作为函数名（`{func, ...}` 上下文）
...
- 条件表达式求值结果必须是布尔值
- 条件语句可任意嵌套
- 循环次数必须是 `int` 类型（`bool` 会被拒绝，即使 Python 中 `isinstance(True, int)` 为真）
